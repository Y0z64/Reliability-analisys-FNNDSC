...
Stages run in order. Stages 1–3 need FNNDSC cluster access.
...
Data root (`BASE_PATH` in every processing script):
...
## Stage 0 - Subject list

Obtained from this [reliability cohort spreadsheet](https://bostonchildrenshospital-my.sharepoint.com/:x:/r/personal/yair_beltran_childrens_harvard_edu/Documents/Attachments/Copy%20of%20reliability%20test_SJ.xlsx?d=wf840aa38f7b94b429c20465c5581eb14&csf=1&web=1&e=NR61XF)
...
Note: `surface_audit.py` saves the error output log for the surface extraction and makes the file quite lengthy.
...
```bash
cd src
uv run jupyter lab      # then run the notebooks, or use something like VScode
```
...
| `assets/artifacts/` | Generated figures. **Ignored by default**, every one is regenerated by a notebook, so a fresh clone starts empty here. |
| `reports/` | Per-subject PDFs and executed notebooks. |
